...
module cpu64_writeback_stage (
    ...
    output wire [`XLEN-1:0]     rd_data_ao,         // Register write data output
    output wire [4:0]           rd_idx_ao,          // Destination register index output
    output wire                 rd_wr_en_ao,        // Register write enable output
...
`endif
    ...
    // Select between load data (processed) and other data sources (ALU, PC+4, CSR)
    // Load instructions use processed memory data, others use data from execute stage
    assign rd_data_ao  = (rd_wr_src_load_i) ? load_data_sliced : rd_data_i;
    
    //===================== Register File Write Control =========//
    // Pass through register index and generate write enable
    assign rd_idx_ao   = rd_idx_i;                              // Destination register index
    assign rd_wr_en_ao = rd_wr_en_i && valid && ~stall_i;      // Write enable: valid instruction and not stalled
    ...
    assign valid_ao        = valid && rst_ni;                  // Output valid: valid instruction and not reset
