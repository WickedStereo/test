...
`ifdef CPU64_DBG_STORES
    always @(*) begin
        if (mem_wr_ao) begin
            $display("[DCD] STORE decode: inst=0x%08h rs2=x%0d rs1=x%0d width=%0d pc=0x%0h", inst_i, rs2_idx_ao, rs1_idx_ao, mem_width_1h_ao, pc_i);
        end
    end
`endif
    ...
    wire M_alu_sel = inst_i[25];    // M-ALU selection bit (bit 25 of instruction)
