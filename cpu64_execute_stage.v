...
`ifdef CPU64_DBG_STORES
    // Debug: Log when a valid store enters/propagates through EXE
    always @(posedge clk_i) begin
        if (rst_ni && ~stall_i && mem_wr_ao) begin
            $display("[EXE] STORE valid pc=0x%0h addr=0x%0h rs2=0x%016h width=%0d", pc_i, I_alu_res[VADDR-1:0], rs2_data, mem_width_1h_i);
        end
    end
`endif
    ...
    cpu64_bypass_unit rs1_bypass_unit (
        ...
        .EXE_mem_read_i     (mem_rd_o),     // EXE stage memory read
        .EXE_rd_wr_en_i     (rd_wr_en_o),   // EXE stage register write enable
        .EXE_valid_i        (valid_o),      // EXE stage valid
        .EXE_rd_idx_i       (rd_idx_o),     // EXE stage destination register
        .EXE_rd_data_i      (rd_data_o),    // EXE stage register data
    ...
    // Branch is taken if instruction is valid, not stalled, and condition is met
    assign branch_o      = valid && ( branch_i || branch_taken || fencei_ao) && ~stall_i;
    assign target_addr_o = fencei_ao ? next_pc_i : (branch_i || branch_taken) ? I_alu_res[VADDR-1:0] : '0;  // Branch target address
    ...
    always @(posedge clk_i) begin
        if (~rst_ni) begin
            ...
            rd_wr_en_o       <= rd_wr_en_i && valid;   // Register write enable
