...
module cpu64_obi_host_driver #(
    ...
    // Allow back-to-back requests: when response arrives (rvalid), new request can be made same cycle
    // CRITICAL FIX: Clear read_outstanding when no read request (prevents stuck state)
    assign read_accepted = (read_outstanding && ~rvalid_i && rd_i) || (read && req && gnt_i);
    // Generate request when: no outstanding read OR response just arrived allowing new request
    assign req = (~read_outstanding || rvalid_i) && ((read || (wr_i && !write_accepted)) || request_stall_r);
...
`ifdef CPU64_DBG_STORES
    // Debug: Log accepted store transactions at OBI host driver
    always @(posedge clk_i) begin
        if (rst_ni) begin
            if (req_o && we_ao && gnt_i) begin
                $display("[OBI_HOST] STORE_ACCEPTED addr=%h be=%0h wdata=%h", addr_ao, be_ao, wdata_ao);
            end
        end
    end
`endif
