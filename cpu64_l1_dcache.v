...
`ifdef CPU64_DBG_STORES
    // Debug: Log CPU-side store hit/miss acceptance at L1 boundary
    always @(posedge clk_i) begin
        if (rst_ni) begin
            if (req_i && we_i && gnt_o) begin
                if (hit) begin
                    $display("[L1] STORE_HIT index=%0d way=%0d addr=%h be=%02h wdata=%h", index, hit_way, addr_i, be_i, wdata_i);
                end else begin
                    $display("[L1] STORE_MISS index=%0d victim=%0d addr=%h be=%02h wdata=%h", index, victim_way, addr_i, be_i, wdata_i);
                end
            end
        end
    end
