...
`ifdef CPU64_DBG_STORES
    // Debug: Log accepted store requests at memory stage boundary
    always @(posedge clk_i) begin
        if (rst_ni) begin
            if (dmem_req_o && dmem_we_ao && dmem_gnt_i) begin
                $display("[MEM] STORE_ACCEPTED addr=%h be=%02h wdata=%h", dmem_addr_ao, dmem_be_ao, dmem_wdata_ao);
            end
        end
    end
`endif
    ...
    always @(posedge clk_i) begin
        if (~rst_ni) begin
            ...
            rd_wr_en_o       <= rd_wr_en_i;       // Register write enable
