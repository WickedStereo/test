...
`ifdef CPU64_DBG_STORES
    // Debug: Log L1-side acceptance of stores
    always @(posedge clk_i) begin
        if (rst_ni) begin
            if (l1_req_o && l1_we_o && l1_gnt_i) begin
                $display("[OBI_RX] L1_STORE_ACCEPTED addr=%h be=%02h wdata=%h", l1_addr_o, l1_be_o, l1_wdata_o);
            end
        end
    end
`endif
